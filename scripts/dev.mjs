...
watch(root,{recursive:true},(_,file)=>{
 if(!file || /^(dist|\.git|node_modules|src)(\/|$)/.test(file))return;
 clearTimeout(timer);timer=setTimeout(()=>{try{build()}catch(e){console.error('Build failed')}},150);
});
const types={'.pdf':'application/pdf','.html':'text/html','.css':'text/css','.js':'text/javascript','.svg':'image/svg+xml','.woff2':'font/woff2','.png':'image/png','.webp':'image/webp','.xml':'application/xml','.txt':'text/plain'};
